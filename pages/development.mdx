...
c8ba242a97be   haven/haven-worker   "python -m app"   5 minutes ago   Up 5 minutes   0.0.0.0:5001->5001/tcp   friendly_mcnulty
```
...
```bash copy
docker run -it -v /usr/local/nvidia:/usr/local/nvidia -v ~/config.json:/app/config.json --gpus all -p 50051:50051 konsti1/peacefulplace-worker /bin/bash
```
